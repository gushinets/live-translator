...
**Baseline ревью:** `5a32ee2a1c3fe81e12b00be404214f0887c27e82`. **Статус:** этап 1 завершён в merged [GitHub PR #14](https://github.com/gushinets/live-translator/pull/14); этап 2 — в merged [#15](https://github.com/gushinets/live-translator/pull/15); этап 3 — в merged [#17](https://github.com/gushinets/live-translator/pull/17), устойчивость outbox дополнена merged [#18](https://github.com/gushinets/live-translator/pull/18); этап 4 — в merged [#19](https://github.com/gushinets/live-translator/pull/19) с follow-up [#20](https://github.com/gushinets/live-translator/pull/20); этап 5 реализован в merged [PR #21](https://github.com/gushinets/live-translator/pull/21), `BACKGROUND_SESSION_CLOSE_ENABLED=false` остаётся default, а device/provider gates указаны отдельно; этап 6 `in-progress` в feature-ветке `feat/unit-economics-reconciliation`, не merged. Документационный PR #13 merged от `158f011d63552d750ae9549fd29c4b3a51d56cf4`.
...
| 5 | [immediate background close и retained conversation](05-background-and-resume.md) | completed ([#21](https://github.com/gushinets/live-translator/pull/21)); device/provider gates remain external | 15 |
| 6 | [сверка, эксплуатационная устойчивость и пилот](06-reconciliation-and-pilot.md) | in-progress (`feat/unit-economics-reconciliation`, not merged) | 9 |
...
| §6 API, idempotency, provisional handoff ACK/deadline, dispatch-vs-cleanup CAS, route-owned disconnect fence, cleanup outbox + PR-2 worker | 2; lifecycle callers 4–5; usage 3 |
...
| §11 limiter/config/reservations/process lifecycle | 1: create/admission config; 2: abortable provider create, dispatch gate/fence, global shutdown deadline, bounded handler drain + worker stop/startup recovery; 4: normal release; 6: reconciliation/report only |
