...
**Создать:** `apps/api/src/persistence/database.ts`, `apps/api/src/persistence/migrations/001-usage-ledger.sql`, `apps/api/src/accounting/UsageLedger.ts`, `apps/api/src/security/AnonymousIdentity.ts`, `apps/api/src/routes/conversations.ts`.  
**Тесты:** `apps/api/test/database.test.ts`, `apps/api/test/conversations.test.ts`, существующие API creation/lease tests и web BackendClient/SessionController tests. New test names явно отмечаются как новые, не как уже существующие.
...
Отдельный owner-authenticated `POST /api/live/session/:localId/cleanup` durably записывает cleanup intent даже если external request уже dispatched, а `openai_session_id` ещё NULL. При позднем provider result ID фиксируется в той же row; наличие cleanup marker запрещает вернуть/активировать SDP и запускает transient Sideband close. Cleanup не требует текущей product version/generation и не расходует creation limiter. Startup recovery повторяет cleanup, если marker и provider ID уже durable; marker без ID после process death остаётся honest dispatched/unknown без automatic create retry. PR 2 уже владеет минимальной `UsageLedger.recordProviderClosed(...)`: Sideband `session.closed` с optional usage/reason/source атомарно фиксирует `state=closed`/`lease_released_at`, затем освобождается memory slot. Schema также хранит независимые `provider_checkpoint_source`, `provider_final_source` и `provider_close_reason_source`; source назначает backend ingestion path, не клиентский payload. PR 3 переиспользует primitive для browser-forwarded close.
...
| A2.5 | Timeout/cleanup до provider result; поздний result; provider success + DB result-commit failure; lost 201; 201→primary startup failure | Любой post-dispatch failure вызывает cleanup route даже при `openai_session_id=NULL`; transaction сохраняет marker и fencing `state=closing`. Если browser исчез, late success фиксирует ID, не возвращает/не активирует SDP и запускает Sideband close. Restart после durable ID повторяет cleanup; restart до получения ID оставляет honest dispatched/unknown marker без automatic create retry. Sideband closed с/без usage сохраняет независимые close/final provenance; lost-201/pre-start failure не replay-ит SDP, не создаёт второго provider и не расходует creation limiter. |
...
- [ ] Ввести две таблицы и repository/ledger интерфейс из спецификации; добавить durable `cleanup_requested_at/reason`, отдельные close/reason/checkpoint/final source fields; уже здесь реализовать минимальный `recordProviderClosed(source, reason?, finalSeconds?)` с source-aware merge и durable admission release, чтобы Sideband cleanup PR 2 не зависел от PR 3; включить FK/WAL и операции без сетевого await внутри transaction.
...
- [ ] Обернуть создание durable registration и idempotency; cleanup route после любого post-dispatch failure сначала коммитит intent и переводит attempt в `closing` даже при неизвестном provider ID. Закрепить A2.5 для cleanup-before-result → browser disappears → late success, provider-success → result-commit-failure, lost 201 и 201→primary-startup failure через fake transient Sideband attach/`session.close`; late result и любой activation path обязаны увидеть marker до выдачи SDP/activation. Cleanup не проходит creation limiter/current generation guard. Добавить restart cases marker+ID / marker-without-ID, Sideband closed-with/without-usage и close→crash A2.6, без automatic external retry.
