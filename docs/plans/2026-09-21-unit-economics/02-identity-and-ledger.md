...
| A2.5 | Metadata reserve fail-closed + server shutdown safety | IndexedDB unavailable/quota/transaction failure до create → `MetadataDeliveryBudget.reserve` fails, UI получает recoverable storage/accounting error, backend/provider calls=0. Existing reserved session может использовать degraded best-effort cleanup. SIGTERM во время Sideband attempt: no new drains/create, active transport bounded-aborted, DB остаётся доступна до worker quiesce, interrupted row остаётся due; restart выполняет cleanup без fabricated failure/success. |
...
- [ ] Сделать `reserve(localId)` hard precondition для create path: IndexedDB/quota/transaction/read-back failure → recoverable local error и zero BackendClient/provider create calls. Existing-session degraded path тестировать отдельно.
