...
| A2.5 | Timeout после dispatch / поздний ответ / provider success + DB result-commit failure / lost 201 после успешного commit | Сохраняется unknown, а не нулевой usage. Поздний provider ID дополняет исходную запись; paused/ended conversation не возвращается к active. Для известного WebRTC provider ID fake Sideband cleanup моделирует attach → `session.close` → closed/timeout; SIP `hangup` не используется. После успешного DB commit повтор того же ID с `responseOutcome=not_received` не replay-ит SDP и не создаёт второго provider, а запускает cleanup существующей session. Новая attempt возможна только после снятия progressing reservation по подтверждённому close/release либо policy timeout. |
| A2.6 | Перезапуск API с живыми/только что закрытыми reservations | До допуска новых созданий прежние действительно незавершённые reservations восстановлены; dispatched creating умершего процесса становится unknown; no-dispatch resume помечается failed. Row с durable `lease_released_at`, `close_confirmed=true` или `state=closed` не rehydrate-ится. Сценарий close commit → crash до in-memory release → restart оставляет slot свободным. |
...
- [ ] Обернуть создание durable registration и idempotency; закрепить оба orphan cases A2.5 (provider-success → result-commit-failure и durable success → lost 201) через fake transient Sideband attach/`session.close`, не SIP hangup и не SDP persistence; добавить startup hydration registry с close→crash regression A2.6, не включая automatic external retry.
