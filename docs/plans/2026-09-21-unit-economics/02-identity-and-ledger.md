...
Отдельный owner-authenticated cleanup idempotent по `localId`; marker ACK означает durable transfer cleanup responsibility server-у. При marker+known-ID SQL atomically ставит `cleanup_next_attempt_at=now`; late-ID commit делает то же, а startup defensive scan считает known-ID NULL-next row immediately due. `CleanupWorker` сериализует localId и expiry через один single-flight, network Sideband ограничен global semaphore (`concurrency=2`) и due batch (`20`). Expiry не пересекается с running attempt: после failure owner сам решает schedule-vs-exhaustion по текущему времени.
...
| A2.5 | Crash before first worker wake; expiry vs in-flight attempt; concurrent wakes; large due backlog | Marker+known-ID commit сохраняет `next=now`; immediate process crash/restart всё равно запускает cleanup. Attempt, начавшаяся до expiry, завершается до exhaustion decision; failure after expiry ставит exhausted без next retry. Marker/late-ID/timer/startup для одного ID дают одну attempt. Backlog >20 запускается batches, network concurrency никогда >2, overflow eventually progresses. |
...
- [ ] Ввести две таблицы/repository: marker+known-ID и late-ID transactions обязаны durable-set `cleanup_next_attempt_at=now`; добавить cleanup retry fields и validated PR-2 config `CLEANUP_WORKER_CONCURRENCY=2`, `CLEANUP_WORKER_BATCH_SIZE=20`. NULL-next known-ID остаётся immediately-due defense-in-depth.
...
- [ ] Закрепить A2.5: marker ACK → crash before wake → restart cleanup; attempt spans expiry without contradictory exhausted/next metadata; concurrent wake gives one attempt/increment; startup backlog 50 rows never exceeds 2 active Sideband attempts, batches ≤20 и все due rows eventually progress. Сохранить unknown-ID expiry, product-deadline independence, budget/idempotency/registration-race cases.
