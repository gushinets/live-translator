...
| A2.5 | Shutdown global budget + dispatched-create cleanup handoff | SIGTERM до dispatch → zero provider calls/no-dispatch. SIGTERM после dispatch немедленно durable ставит backend-only `server_shutdown` cleanup marker (ID может NULL); late success не активируется, ID persists и cleanup due. Drain timeout abort не становится definitive failure. Near-worst-case 18s drain + active worker stop укладывается в global 40s и Compose 45s grace; restart выполняет Sideband cleanup без external create retry. |
...
- [ ] Переписать `server.ts`: global deadline `SERVER_SHUTDOWN_TIMEOUT_MS=40000`; dispatch gate; при SIGTERM durable cleanup-fence всех tracked dispatched rows (`server_shutdown`, closing, immutable retry expiry, next=now if ID known) **до drain**; drain ≤18000 при живых Worker/DB; `CleanupWorker.stop(remainingMs)`; HTTP/DB close в том же deadline. Compose `stop_grace_period: 45s`.
...
- [ ] Собрать API Docker image и container SIGTERM smokes: registration→signal→0 provider calls; dispatched→shutdown marker ID NULL→late success→no activation→restart Sideband cleanup; worst-case drain + active Sideband stop exits <40s/<45s grace. Проверить DB/volume/migrations и общий regression suite.
