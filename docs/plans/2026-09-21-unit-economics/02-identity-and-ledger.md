...
Отдельный owner-authenticated cleanup idempotent по `localId`; marker ACK означает durable transfer cleanup responsibility server-у. `CleanupWorker` в single-process MVP сериализует marker/late-ID/timer/startup triggers через in-process single-flight по `localId`; due scan пропускает running rows, а retry metadata увеличивается один раз на фактическую Sideband attempt. Каждый tick сначала expiry-scan-ит **все** cleanup-marked non-terminal rows независимо от provider ID и при deadline ставит `cleanup_retry_exhausted_at`/unknown без fake close/release; затем Sideband due-scan обрабатывает только rows с известным ID. Client outbox/capacity semantics остаются из spec.
...
| A2.5 | CleanupWorker concurrent wake/scan; marker without provider ID; retry expiry | Marker/late-ID/timer/startup одновременно будят один localId — активна ровно одна Sideband attempt, `cleanup_attempt_count` растёт на 1 и next backoff записывается один раз. Row с cleanup marker + `openai_session_id=NULL` без late result всё равно попадает в expiry-only scan через 7 дней: `cleanup_retry_exhausted_at` set, outcome unknown, без fabricated close/final/release. |
...
- [ ] Добавить conversation routes/cookie/client context; реализовать `CleanupWorker` с `Map/Set<localId>` single-flight для marker/late-ID/timer/startup wake, due-scan skip running rows, transactional one-attempt retry-metadata update и **expiry-only scan всех cleanup-marked rows независимо от provider ID**. Client `CleanupIntentOutbox`, capacity и delivery rules — по spec.
- [ ] Закрепить A2.5: concurrent marker wake + periodic scan + startup/late-ID дают одну Sideband attempt и один increment; marker-without-ID → no late result → 7-day expiry → exhausted/unknown без close/release. Сохранить marker ACK→Sideband fail→server retry, product-deadline independence, budget capacity, definitive-error, registration-race 404 и first-reason-wins cases.
