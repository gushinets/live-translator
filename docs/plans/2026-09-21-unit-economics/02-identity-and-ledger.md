...
**Создать:** `apps/api/src/persistence/database.ts`, `apps/api/src/persistence/migrations/001-usage-ledger.sql`, `apps/api/src/accounting/UsageLedger.ts`, `apps/api/src/security/AnonymousIdentity.ts`, `apps/api/src/routes/conversations.ts`, `apps/web/src/session/CleanupIntentOutbox.ts`.  
**Тесты:** `apps/api/test/database.test.ts`, `apps/api/test/conversations.test.ts`, новый `apps/web/src/session/CleanupIntentOutbox.test.ts`, существующие API creation/lease tests и web BackendClient/SessionController tests. New test names явно отмечаются как новые, не как уже существующие.
...
Отдельный owner-authenticated `POST /api/live/session/:localId/cleanup` durably записывает cleanup intent всякий раз, когда dispatched attempt больше не разрешено активировать: technical failure, hidden-while-creating, End/cancel, replacement или lost response; `openai_session_id` может быть ещё NULL. При позднем provider success ID фиксируется в той же row; наличие cleanup marker запрещает вернуть/активировать SDP и запускает transient Sideband close. Если исходный create заканчивается definitive provider error без ID, cleanup-pending row переходит `closing→failed`, durable reservation release фиксируется без fabricated final и Sideband не вызывается. Cleanup не требует текущей product version/generation и не расходует creation limiter. `CleanupIntentOutbox` сохраняет intent до HTTP send, повторяет до SQL-commit ACK и flush-ится на foreground/reconnect прежде новой attempt того же conversation; network/DB failure не открывает gates. Startup recovery повторяет cleanup, если marker и provider ID уже durable; marker без ID после process death остаётся honest dispatched/unknown без automatic create retry. PR 2 уже владеет минимальной `UsageLedger.recordProviderClosed(...)`: Sideband `session.closed` с optional usage/reason/source атомарно фиксирует `state=closed`/`lease_released_at`, затем освобождается memory slot. Schema также хранит независимые `provider_checkpoint_source`, `provider_final_source` и `provider_close_reason_source`; source назначает backend ingestion path, не клиентский payload. PR 3 переиспользует primitive для browser-forwarded close.
...
| A2.5 | Post-dispatch failure или lifecycle abandonment до provider result; cleanup HTTP loss/DB outage; поздний success/definitive failure | Hidden/End/cancel/replacement/lost-response после dispatch enqueue cleanup даже при `openai_session_id=NULL`; marker fences `state=closing`. Cleanup outbox retry до commit ACK не создаёт provider и перед same-conversation retry flush-ится первым. Late success фиксирует ID и закрывается через Sideband без SDP activation. Definitive create failure без ID переводит `closing→failed`, durable-release reservation и оставляет final NULL. Restart marker+ID повторяет cleanup; marker без ID остаётся honest unknown без automatic create retry. |
...
- [ ] Ввести две таблицы и repository/ledger интерфейс из спецификации; добавить durable `cleanup_requested_at/reason` с lifecycle allowlist, отдельные close/reason/checkpoint/final source fields; уже здесь реализовать минимальный `recordProviderClosed(source, reason?, finalSeconds?)` с source-aware merge и durable admission release, чтобы Sideband cleanup PR 2 не зависел от PR 3; включить FK/WAL и операции без сетевого await внутри transaction.
- [ ] Добавить conversation routes, cookie и передачу local attempt context через BackendClient/LiveClient; реализовать `CleanupIntentOutbox` в IndexedDB с commit-ACK retry/backoff/foreground flush и degradation/loss diagnostics. Не менять language routing и background policy до PR 5.
- [ ] Обернуть создание durable registration и idempotency; cleanup route при любом post-dispatch abandonment сначала коммитит intent и переводит attempt в `closing` даже при неизвестном provider ID. Закрепить A2.5 для hidden/End/cancel/replacement, cleanup-before-result → browser disappears → late success, provider-success → result-commit-failure, lost 201 и 201→primary-startup failure через fake transient Sideband attach/`session.close`; late result и activation paths обязаны увидеть marker. Cleanup не проходит creation limiter/current generation guard. Добавить definitive-error-after-cleanup (`closing→failed`, durable release, final NULL), cleanup-request HTTP loss/DB recovery, restart marker+ID/marker-without-ID, Sideband closed-with/without-usage и close→crash A2.6, без automatic external retry.
