...
| A2.6 | API restart: ambiguous dispatch, provisional handoff, active handoff и cleanup reservations | До admission recovery разделяет: dispatched/no committed result → unknown/no retry; result-committed + unacked + future handoff deadline → сохранить `creating`+reservation/watchdog и разрешить owner handoff ACK; expired unacked → `handoff_timeout` fence/cleanup before admission; acknowledged `active` → сохранить. Resume pending claim сохраняется только для valid provisional/acknowledged row и unexpired claim; no-dispatch/ambiguous/expired/fenced rollback-ится. Closed/released row не rehydrate-ится. |
...
| ID | Условие/сценарий | Ожидаемый результат |
|---|---|---|
| A2.10 | Resume claim/handoff/complete ordering и повторы | Claim не вызывает creator/slot. Matching first dispatch один. `/resume/complete` transaction требует `handoff_acknowledged_at IS NOT NULL`, provider `state=active`, pending claim, no cleanup и valid deadlines. Complete-before-handoff → 409/no mutation; handoff-first → complete succeeds once; handoff-timeout/cleanup-first/terminal provider → complete никогда не активирует conversation. Duplicate same operation idempotent, stale/opposite payload не создаёт provider. |
