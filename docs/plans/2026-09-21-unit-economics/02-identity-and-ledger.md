...
Отдельный owner-authenticated `POST /api/live/session/:localId/cleanup` закрывает existing known-provider attempt при lost 201 либо post-201/pre-primary-ready failure (`setRemoteDescription`, data channel, pre-`session.started`) без SDP replay/второго create. Cleanup не требует текущей product version/generation и не расходует creation limiter. PR 2 уже владеет минимальной `UsageLedger.recordProviderClosed(...)`: Sideband `session.closed` с optional usage/reason/source атомарно фиксирует `state=closed`/`lease_released_at`, затем освобождается memory slot; PR 3 переиспользует primitive для browser-forwarded close.
...
| A2.5 | Timeout после dispatch / поздний ответ / provider success + DB result-commit failure / lost 201 / 201→primary startup failure | Сохраняется unknown, а не нулевой usage. Поздний provider ID дополняет исходную запись; paused/ended conversation не возвращается к active. Для известного WebRTC provider ID fake cleanup моделирует Sideband attach → `session.close` → `session.closed`/timeout; SIP `hangup` не используется. Lost-201 и `setRemoteDescription`/data-channel/pre-start failure вызывают idempotent cleanup route, который не replay-ит SDP, не создаёт второго provider, не проверяет current product generation/version и не расходует creation limiter. Sideband closed с usage и без usage оба проходят PR-2 `recordProviderClosed`. Новая attempt возможна только после durable release либо policy timeout. |
| A2.6 | Перезапуск API с живыми/только что закрытыми reservations | До допуска новых созданий прежние действительно незавершённые reservations восстановлены; dispatched creating умершего процесса становится unknown; no-dispatch resume помечается failed. Row с durable `lease_released_at` или `state=closed` после `recordProviderClosed`/owner release не rehydrate-ится. Sideband/browser source сохраняется отдельно. Сценарий close commit → crash до in-memory release → restart оставляет slot свободным. |
...
- [ ] Ввести две таблицы и repository/ledger интерфейс из спецификации; уже здесь реализовать минимальный `recordProviderClosed(source, reason?, finalSeconds?)` и durable admission release, чтобы Sideband cleanup PR 2 не зависел от PR 3; включить FK/WAL и операции без сетевого await внутри transaction.
...
- [ ] Обернуть создание durable registration и idempotency; добавить отдельный cleanup route и закрепить A2.5 для provider-success → result-commit-failure, lost 201 и 201→primary-startup failure через fake transient Sideband attach/`session.close`, не SIP hangup и не SDP persistence. Cleanup не проходит creation limiter/current generation guard. Добавить startup hydration registry с Sideband closed-with/without-usage и close→crash regression A2.6, не включая automatic external retry.
