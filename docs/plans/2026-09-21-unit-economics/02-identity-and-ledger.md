...
| A2.5 | Dispatch-vs-cleanup CAS + HTTP disconnect handoff | Pre-dispatch cleanup и dispatch гоняются через один CAS: cleanup-first → durable ACK/no-dispatch/release и provider calls=0; dispatch-first → cleanup sees dispatched and closing/worker path. После dispatch client socket abort/premature response close сначала durable ставит backend-only `client_disconnected` marker (ID может NULL), затем AbortSignal; late success persists ID, never activates/returns SDP, Sideband cleanup scheduled/recovered. |
