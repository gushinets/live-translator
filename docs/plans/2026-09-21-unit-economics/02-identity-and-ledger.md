...
| A2.5 | Shutdown global budget + abortable dispatched-create handoff | SIGTERM до dispatch → zero provider calls/no-dispatch. После dispatch durable `server_shutdown` fence ставится до drain. Fake creator получает route-owned AbortSignal; drain timeout/deadline вызывает abort, promise/handler settles bounded, но outcome остаётся ambiguous (`closing`, cleanup marker, no fake failed/release). Late success не активируется; restart cleanup/unknown recovery сохраняется. Worst-case shutdown укладывается <40s/<45s grace. |
