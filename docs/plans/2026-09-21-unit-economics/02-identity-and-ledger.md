...
`server.ts` после PR 2 становится владельцем coordinated process lifecycle: HTTP server, DB, CleanupWorker, shared create-dispatch gate и in-flight provider-create tracker. SIGTERM/SIGINT закрывает dispatch gate и HTTP accept; registered-but-not-dispatched handler durable-завершается no-dispatch/503 без OpenAI. Уже provider-dispatched handlers bounded-drain-ятся до `SERVER_SHUTDOWN_DRAIN_MS=20000` **при живых CleanupWorker и DB**, включая late success/result-commit ambiguity → cleanup handoff. Только после этого worker stop/quiesce, затем DB close. Compose PR 2 задаёт `stop_grace_period: 30s`.
...
| A2.5 | Metadata budget races + shutdown-vs-create ordering | Cross-tab reserve/dispatch-marker invariants сохраняются. SIGTERM между local registration и provider dispatch закрывает shared dispatch gate: creator calls=0, row/reservation получают durable no-dispatch outcome/release. SIGTERM после committed provider dispatch bounded-drain-ит handler при живых Worker/DB; provider success + result-commit failure успевает durable передать cleanup/unknown worker-у. Worker останавливается только после handler drain/handoff; restart не повторяет external create. |
...
- [ ] Переписать `server.ts` на explicit resource ownership/shutdown: shared create-dispatch gate + in-flight dispatched-handler tracker; SIGTERM закрывает gate/HTTP accept, durable-abort-ит registered-not-dispatched rows, bounded-drain-ит provider-dispatched handlers (`SERVER_SHUTDOWN_DRAIN_MS=20000`) при живых Worker/DB, затем worker stop и DB close. `infra/docker-compose.yml` добавить `stop_grace_period: 30s`.
...
- [ ] Собрать API Docker image; после Sideband dependency smoke выполнить container SIGTERM scenarios: registration→signal→zero provider calls; dispatched create→signal→provider success/result-commit failure→cleanup handoff while Worker/DB alive→restart recovery. Проверить `stop_grace_period: 30s`, DB/volume/migrations и общий regression suite.
