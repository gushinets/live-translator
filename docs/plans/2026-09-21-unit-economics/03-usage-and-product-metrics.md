...
UsageOutbox coalesces metadata одной сессии и **переиспользует PR-2 `MetadataDeliveryBudget`**, не вводит собственный cap. Usage pending/final/app seq удерживают envelope; provider/no-provider terminal outcome + ACK всех client metadata + local producer finalized позволяют shared release predicate освободить envelope. Historical server ledger row не удаляется. Usage metadata не может вытеснить cleanup и не держит envelope после полного ACK/finalization.
...
| A3.6 | Сеть/БД отключены во время final; shared budget/release lifecycle | Usage retry после foreground не дублирует merge. Budget keyed by localId; cleanup priority. Pending usage удерживает envelope. После terminal/no-provider outcome + cleanup/usage ACK + app finalization shared release возвращает count к baseline; historical server ledger сохраняется. |
