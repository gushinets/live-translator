...
UsageOutbox coalesces metadata одной сессии, но наследует PR-2 capacity contract: общий logical budget считается по unique `localId` across cleanup+usage, а cleanup priority выше usage. Usage для уже известного localId не создаёт второй budget slot; при capacity pressure usage coalesce/throttle раньше cleanup, cleanup entry не evict-ится. Final/checkpoint не теряются внутри допустимого envelope и provider create из reporter не запускается.
...
| A3.6 | Сеть/БД отключены во время final; shared budget на capacity | Usage metadata retry после foreground не дублирует merge. Budget считается unique `localId`; cleanup для current live attempt admissible даже при capacity, usage не evict-ит cleanup. При 1000 localIds новый provider dispatch уже запрещён PR 2. |
...
- [ ] Реализовать UsageOutbox/reporter поверх PR-2 unique-localId budget: usage coalesce/throttle до cleanup, cleanup никогда не evict-ится; проверить network outage и budget-at-capacity A3.6.
