...
| A3.4 | Browser-forwarded closed без usage / отрицательные или бесконечные seconds | Browser observation маркируется `close_confirmation_source=browser`, не выдаётся за независимую provider authority и не превращается в final0. Он вызывает PR-2 `recordProviderClosed`, поэтому cooperative admission release остаётся атомарным/durable даже при `provider_final_seconds=NULL`; metric anomaly не теряет валидную причину close, usage остаётся partial/unknown. Security-sensitive quota из этого сигнала не выводится. |
...
- [ ] Расширить parser/event contract и accounting binding; browser close передавать в PR-2 `recordProviderClosed(source=browser, ...)`, не дублировать durable release логику. Проверить source marking и partial usage A3.4. Сохранить teardown-before-untrusted-callback safety, не привязывая sink к current product generation.
