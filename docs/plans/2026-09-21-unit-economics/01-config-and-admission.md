...
Формат чисел — только десятичные цифры, positive safe integer. Дополнительный технический предел `LIVE_SESSION_RATE_WINDOW_MS <= 2147483647` соответствует timer range встроенного MemoryStore, не является новым продуктовым лимитом: [официальная документация limiter](https://express-rate-limit.mintlify.app/reference/configuration#windowms). Compose использует unset-only default (`-`, не `:-`), чтобы пустое заданное значение не стало silent fallback.
