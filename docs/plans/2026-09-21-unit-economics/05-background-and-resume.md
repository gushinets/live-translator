...
| A5.6 | Две вкладки/duplicate/opener-created tab, hidden/frozen owner, reload/одновременный resume / stale End | Document удерживает exclusive Web Lock `client-instance:<id>` всё время жизни до snapshot access/использования. Обычный reload сохраняет ID после ухода старого document; duplicate/opener clone со скопированным sessionStorage не может получить lock даже если owner hidden/frozen, ротирует ID и только затем получает отдельный IndexedDB snapshot key. BroadcastChannel не является арбитром. При отсутствии Web Locks automatic same-ID restore fail-closed. Для одного paused conversation только один claim; stale mutation получает 409. Usage предыдущей session всё ещё принимается без проверки текущей product version. |
...
| A5.14 | После dispatch отказ create; либо 201 получен, но `setRemoteDescription`/data channel/pre-`session.started`/restore ACK/autoplay не дали usable primary close path | Local close/teardown и abort не открывают gates. Если provider ID известен, owner вызывает idempotent cleanup route; он не зависит от product generation/version или creation quota и использует transient Sideband `session.close`, когда primary close невозможен. Failed/closing/unknown честно сохранены; новый claim не обходит progressing attempt. Lost complete ACK сначала проверяется read, не вызывает второе создание. |
...
- [ ] Реализовать versioned snapshot с tab-scoped `clientInstanceId` в sessionStorage, document-lifetime exclusive Web Lock и IndexedDB key `(clientInstanceId, conversationId)`; duplicate/opener clone, включая hidden/frozen owner, обязан ротировать скопированный ID до snapshot access, а отсутствие Web Locks отключает automatic same-ID restore. Затем resumeAttemptId, pause/claim/complete/abort, restore transition и startup ACK sequence. Подключить PR-2 cleanup route; проверить two-tabs + reload + duplicate/opener/frozen-owner A5.6 и post-201 pre-primary-ready cleanup A5.14.
