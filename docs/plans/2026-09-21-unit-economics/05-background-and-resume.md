...
На 2026-09-26: `pnpm test` — 1098/1098, `pnpm typecheck` — exit 0. Предыдущий head draft PR `d9481c0` фиксировал 1097/1097; CI run 36188651465 прошёл четыре job на том коммите и не подтверждает эту правку. Regression покрывают rejected/pending claims, lost pause ACK, недоступный storage, retained End, no-provider proof, usage identity и паузу ON-policy разговора, если cached flag-off create скрывается при доступных Web Locks/IDB: строка не остаётся active без product deadline, reload видит paused. G1 mock report воспроизведён ранее скриптом `node apps/api/scripts/g1-mock-report.mjs`; это synthetic fixture без provider charges. Это локальный unit suite, не CI/device/provider evidence и не закрытие Gate G2. Server regression для provisional/handed-off resume на точной claim boundary и browser opener-clone lock находятся в `apps/api/test/UsageLedger.test.ts` и `apps/web/tests/e2e/recovery.spec.ts`.
