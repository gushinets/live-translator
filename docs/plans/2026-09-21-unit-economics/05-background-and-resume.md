...
| A5.6 | Две вкладки с разными conversations, reload/одновременный resume / stale End | У каждой вкладки свой `clientInstanceId` в sessionStorage и свой IndexedDB snapshot key; reload восстанавливает только свой pointer, новая вкладка не выбирает origin-wide «последний» conversation. Для одного paused conversation только один claim; stale mutation получает 409. Usage предыдущей session всё ещё принимается без проверки текущей product version. |
...
- [ ] Реализовать versioned snapshot с tab-scoped `clientInstanceId` в sessionStorage и IndexedDB key `(clientInstanceId, conversationId)`, затем resumeAttemptId, pause/claim/complete/abort, отдельную restore transition и повтор startup ACK sequence на новом provider. Подключить server contract PR 2; проверить two-tabs + reload A5.6, local cleanup при потерянном abort/complete ACK и retry без нового retention A5.13–A5.15.
