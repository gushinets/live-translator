...
| A5.6 | Две вкладки/duplicate/opener-created tab, reload/одновременный resume / stale End | Перед snapshot access вкладка подтверждает live ownership `clientInstanceId` через Web Lock или эквивалентный BroadcastChannel handshake. Обычный reload сохраняет ID при отсутствии другого live owner; duplicate/opener clone со скопированным sessionStorage обнаруживает collision, ротирует ID и получает отдельный IndexedDB snapshot key до чтения/записи. Новая вкладка не выбирает origin-wide «последний» conversation. Для одного paused conversation только один claim; stale mutation получает 409. Usage предыдущей session всё ещё принимается без проверки текущей product version. |
...
- [ ] Реализовать versioned snapshot с tab-scoped `clientInstanceId` в sessionStorage, live-owner collision check (Web Lock или эквивалентный BroadcastChannel handshake) и IndexedDB key `(clientInstanceId, conversationId)`; duplicate/opener clone обязан ротировать скопированный ID до snapshot access. Затем resumeAttemptId, pause/claim/complete/abort, отдельную restore transition и повтор startup ACK sequence на новом provider. Подключить server contract PR 2; проверить two-tabs + reload + duplicate/opener A5.6, local cleanup при потерянном abort/complete ACK и retry без нового retention A5.13–A5.15.
