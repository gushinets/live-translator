...
| A5.15 | Kill/API restart; provisional/handed-off resume; claim expiry cleanup; cleanup-vs-complete | Restart до valid handoff deadline сохраняет provisional creating/resuming claim; restart после handoff ACK до complete сохраняет provider active/resuming claim. При claim/retention/product expiry любой dispatched non-terminal provider atomically cleanup-fence-ится (`resume_claim_expired` if first reason) и переводится closing **в той же transaction**, где claim→expired и conversation→paused/ended; known ID Sideband due. Complete-before-handoff/after expiry/cleanup conflicts; handoff-first complete succeeds only before deadline. |
