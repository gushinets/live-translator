...
| `cleanup_requested_at`, `cleanup_reason` | Nullable server UTC и nullable TEXT enum: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`, `server_shutdown`, `client_disconnected`. После dispatch marker является activation fence; reason NULL iff marker NULL. `server_shutdown` и `client_disconnected` — backend-only internal reasons и не принимаются из client cleanup payload |
...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent. Client allowlist: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`; backend-only `server_shutdown`/`client_disconnected` rejected из payload. Idempotency key — `localId`, first committed reason wins. Если row ещё зарегистрирована, но `provider_request_dispatched_at IS NULL`, cleanup transaction превращает её в definitive no-dispatch terminal row (`cleanup_requested_at/reason`, `state=failed`, `lease_released_at=now`, retry expiry NULL) и ACK-ит; creator после этого запрещён. После dispatch marker→`closing` и immutable retry expiry; ACK только после SQL commit |
