...
| `cleanup_requested_at`, `cleanup_reason` | Nullable server UTC и nullable TEXT enum: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`. После dispatch marker является activation fence; reason NULL iff marker NULL |
...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent; reason: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled` или `replacement`. Не создаёт provider, не требует актуальной product generation/version и не расходует creation limiter. После dispatch атомарно сохраняет marker и переводит non-terminal attempt в `closing`; это activation fence даже при `openai_session_id=NULL`. ACK возвращается только после SQL commit marker. Если ID известен и primary close path недоступен, backend использует transient Sideband `session.close` |
...
Cleanup не ограничен technical failure и не требует уже известного provider ID. Как только **dispatched attempt больше не разрешено становиться active provider**, owner вызывает `POST /api/live/session/:localId/cleanup`, даже когда `openai_session_id` ещё NULL. Это включает technical failure, hidden во время creating, explicit End/cancel, bootstrap replacement/cancellation и lost/non-received response. Endpoint сначала durably записывает `cleanup_requested_at/reason` и атомарно переводит non-terminal attempt в `closing`; marker является terminal fence для любой последующей provider activation. Если `openai_session_id` уже известен и нет пригодного primary пути для `session.close`, backend запускает transient Sideband cleanup; если ID ещё NULL, intent остаётся pending и request не считается выполненным закрытием. К техническим случаям относятся post-201 `setRemoteDescription`, data-channel или pre-`session.started` failure в initial/bootstrap/resume. SDP не хранится и не replay-ится, второй provider create запрещён. Cleanup route не сверяет current product generation/version и не расходует creation limiter: stale pause/End/rate-limit не должны мешать закрытию уже существующего provider resource.

Late provider-result path обязан в одной транзакционной последовательности записать известный `openai_session_id`/creation result и проверить durable cleanup intent. Если `cleanup_requested_at` уже есть, SDP не возвращается/не активируется даже при успешном provider result: attempt остаётся/становится `closing`, после commit backend запускает Sideband cleanup известного ID. Если тот же in-flight external create вместо success возвращает **definitive creation failure** (не timeout/ambiguous outcome) и provider ID так и не появился, cleanup-pending row переходит `closing → failed`; transaction фиксирует failure metadata и `lease_released_at`, оставляет `provider_final_seconds=NULL`, затем освобождается memory reservation. Sideband не вызывается, потому что provider resource подтверждённо не создан. Любой server-side activation/complete predicate обязан требовать `cleanup_requested_at IS NULL`; cleanup marker не очищается для reuse attempt. Browser может к этому моменту исчезнуть. Если процесс рестартовал **после durable ID/result commit, но до Sideband close**, startup/request-time recovery видит `cleanup_requested_at + openai_session_id` и повторяет idempotent cleanup. Если процесс умер **до получения/фиксации provider ID**, новый процесс не может восстановить потерянный response старого external request: row остаётся dispatched/unknown с cleanup intent, automatic create retry запрещён, reservation удерживается консервативно до policy/lease expiry или будущего независимого observation. До подтверждённого cleanup новая provider attempt для conversation не обходит progressing row. Если Sideband attach/close или фиксация результата не подтверждены, usage/outcome остаётся `unknown`, не нулевым.

Клиентская доставка cleanup intent reliable-enough и входит уже в PR 2. До первой HTTP попытки intent сохраняется в отдельный metadata-only `CleanupIntentOutbox` в IndexedDB (`localId`, `conversationId`, allowlisted reason, queued/attempt timestamps); это **не** provider-create queue и она никогда не вызывает OpenAI. Запись удаляется только после cleanup ACK, который следует после SQL commit marker, либо после read-back подтверждённого terminal/released state. При network/5xx/DB failure запрос повторяется idempotently с bounded backoff 1/2/4/8/16/30 секунд + jitter, пока document активен, и снова на foreground/reconnect/reload при той же identity. Перед новой provider attempt того же conversation клиент сначала flush-ит pending cleanup intents; пока старая attempt остаётся progressing и cleanup commit не подтверждён, новый external create не отправляется. Ошибка cleanup HTTP никогда не возвращает gates и не отменяет локальный abandonment.

Cleanup queue использует тот же metadata budget, что §7.2: до 7 дней и общий максимум 1000 local-session records; PR 2 применяет эти пределы сразу, а PR 3 добавляет usage records в тот же budget. Недоступный IndexedDB означает явную `cleanup_delivery_degraded` диагностику и best-effort retry в памяти, не имитацию commit. TTL/eviction или необратимая 401/403/404 из-за утраченной identity фиксируются как `cleanup_delivery_lost` lifecycle/accounting anomaly; старые IDs не перепривязываются к новой cookie. Такой drop не оживляет attempt и не разрешает same-conversation automatic retry после истёкших product/retention deadlines.
...
Unused/not-yet-connected replacement **без** отправленного external POST можно уничтожить abrupt: это не provider session с утраченным final. Если replacement/cancel/End происходит после dispatch, но до usable active provider, client инвалидирует старую attempt и enqueue-ит cleanup intent `replacement`, `cancelled` или `user_end` до/вместе с product transition; late provider success уже не может активироваться. Disconnect helper не должен называться «provider hangup», когда он делает лишь transport teardown/release.
...
Visibility tracking регистрируется до первого provider request и проверяет initial visibility. Hidden safety prelude выполняет local gates-off, завершает active interval, инвалидирует операции восстановления и инициирует provider close без ожидания очереди lifecycle, mute ACK или HTTP. Если hidden застал уже-dispatched `creating` attempt без usable primary, клиент enqueue-ит cleanup intent `hidden`; product pause не ждёт cleanup ACK, а late create result fenced marker-ом и не активируется.
...
End выключает local capture/output сразу и делает terminal product transition. Для уже usable primary provider выполняется normal graceful close. Если End/cancel застал dispatched, но ещё не usable/activatable attempt, до/вместе с transition enqueue-ится cleanup intent `user_end`/`cancelled`; поздний create result не может вернуть conversation в active. Pause deadline проверяется backend при resume и reconciliation; late usage не продлевает retention. Remote unexpected close/transport failure сохраняет причину и usage, переводит продукт в безопасное состояние; бесконечный auto-reconnect не вводится.
