...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent. Idempotency key — `localId`: **first committed cleanup reason wins**; последующие requests с другой allowlisted reason ACK-ятся как тот же intent без 409 и без переписывания reason. Не создаёт provider, не требует актуальной product generation/version и не расходует creation limiter. После dispatch атомарно сохраняет marker и переводит non-terminal attempt в `closing`; ACK только после SQL commit marker |
...
Клиентская доставка cleanup intent reliable-enough и входит уже в PR 2. До первой HTTP попытки intent сохраняется в отдельный metadata-only `CleanupIntentOutbox` в IndexedDB (`localId`, `conversationId`, first cleanup reason, queued/attempt timestamps); это **не** provider-create queue и она никогда не вызывает OpenAI. Outbox coalesce-ит по `localId`, first stored reason wins. Entry удаляется только после: (a) cleanup ACK после SQL commit marker; (b) read-back той же attempt показывает `cleanup_requested_at IS NOT NULL`; (c) backend-confirmed provider terminal state доказывает, что cleanup уже не нужен — definitive `state=failed` без provider ID либо `state=closed`/`close_confirmed=true`; или (d) original create доказанно завершился pre-dispatch/no-external-create. **`lease_released_at` и успешный admission DELETE сами по себе никогда не подтверждают cleanup delivery.** При network/5xx/DB failure запрос retry-ится idempotently с bounded backoff 1/2/4/8/16/30 секунд + jitter, пока document активен, и снова на foreground/reconnect/reload при той же identity. Перед новой provider attempt того же conversation клиент сначала flush-ит pending cleanup intents; пока старая attempt остаётся progressing и cleanup commit не подтверждён, новый external create не отправляется. Cleanup HTTP failure никогда не возвращает gates.

Cleanup может обогнать registration исходного create. Для locally-known create, который уже инициирован, 404/409 `attempt_not_found` не является terminal: пока create pending/ambiguous, cleanup retry-ится. Если row появляется, следующий retry коммитит marker до late activation. После reload без исходного promise retry продолжается до product/max-conversation deadline (и не дольше metadata TTL), блокируя same-conversation new create. 401/403 либо 404 становятся terminal identity/ownership loss только после отдельного read-back conversation/owner контекста, подтверждающего потерю доступа. Если original create сам доказанно возвращает pre-dispatch rejection/no-external-create, cleanup entry можно закрыть как `not_dispatched`.

Cleanup queue использует тот же metadata budget, что §7.2: до 7 дней и общий максимум 1000 local-session records; PR 2 применяет эти пределы сразу, а PR 3 добавляет usage records в тот же budget. Недоступный IndexedDB означает явную `cleanup_delivery_degraded` диагностику и best-effort retry в памяти, не имитацию commit. TTL/eviction либо подтверждённая утрата identity/ownership фиксируются как `cleanup_delivery_lost`; bare 404 во время registration race к этой категории не относится. Старые IDs не перепривязываются к новой cookie.
