...
| `cleanup_requested_at`, `cleanup_reason` | Nullable server UTC и nullable TEXT enum: `response_not_received`, `primary_startup_failed`, `abandoned_connect`. После dispatch marker является activation fence; reason NULL iff marker NULL |
...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent; reason: `response_not_received`, `primary_startup_failed` или `abandoned_connect`. Не создаёт provider, не требует актуальной product generation/version и не расходует creation limiter. После dispatch атомарно сохраняет marker и переводит non-terminal attempt в `closing`; это activation fence даже при `openai_session_id=NULL`. Если ID известен и primary close path недоступен, backend использует transient Sideband `session.close` |
