...
| Browser/API исчез / abort HTTP потерян / claim истёк | Startup/read/mutation классифицирует durable row, а не blindly abort-ит claim: no-dispatch pending → failed + paused/ended; dispatched без committed result → unknown + paused/ended; result-committed provisional handoff с future handoff **и claim** deadlines, no cleanup → сохранить `resuming`/`resume_outcome=pending`/provider `creating`, browser ещё может handoff ACK; committed handoff + valid claim → сохранить provider `active` + conversation `resuming`, browser ещё может `/resume/complete`; expired handoff/claim или cleanup fence → cleanup/abort/paused-or-ended по существующему contract | Старый ID читает durable outcome; automatic external create нет |
...
При старте процесса recovery/classification выполняется **до допуска новых созданий** и различает durable create/handoff state. (1) `provider_request_dispatched_at IS NOT NULL` + `creation_completed_at IS NULL`: внешний результат не закоммичен, row остаётся honest ambiguous/`unknown` (либо `closing`, если cleanup marker уже есть), external create не повторяется. (2) `creation_completed_at IS NOT NULL`, known provider ID, `handoff_acknowledged_at IS NULL`, `cleanup_requested_at IS NULL`, `handoff_ack_deadline_at > now`: row сохраняется provisional `creating` вместе с reservation; startup регистрирует durable watchdog, а browser может ещё ACK handoff. Для resume это допустимо только если matching `conversation.status=resuming`, `resume_outcome=pending` и `resume_claim_expires_at > now`; такой claim **не rollback-ится**. (3) Та же result-committed unacked row при `handoff_ack_deadline_at <= now` до admission atomically получает backend-only `handoff_timeout` cleanup fence/`closing`/`next=now`; late ACK запрещён. (4) `handoff_acknowledged_at IS NOT NULL`/provider `state=active` сохраняется как handed-off; pending resume claim при валидных deadlines остаётся `resuming` и может `/resume/complete`. Cleanup-marked rows остаются closing/worker responsibility; `lease_released_at IS NOT NULL` или terminal closed rows не rehydrate-ятся. Resume claim, который no-dispatch, ambiguous without committed result, expired или cleanup-fenced, rollback-ится в paused/ended по §10.3 без продления retention. Expired reservation сама по себе не превращает provider outcome в closed.
