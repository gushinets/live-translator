...
| `cleanup_attempt_count`, `cleanup_last_attempt_at`, `cleanup_next_attempt_at` | INTEGER NOT NULL DEFAULT 0 и nullable server UTC timestamps для durable server retry schedule; обновляются только backend cleanup worker-ом |
| `cleanup_retry_expires_at`, `cleanup_retry_exhausted_at` | Nullable server UTC. Для dispatched cleanup default retry deadline = `cleanup_requested_at + 7 days`; product/retention deadlines его не сокращают. Exhausted не означает provider closed |
...
После cleanup marker SQL commit ответственность за provider termination переходит backend. PR 2 запускает server cleanup worker в том же single-API process. Все trigger-ы — marker commit, late provider-ID commit, periodic due scan и startup recovery — проходят через in-process **single-flight keyed by `localId`** (`Map<localId, Promise>`/эквивалент): если cleanup этого localId уже выполняется, новый trigger присоединяется/схлопывается, а due-scan пропускает running row. Поэтому одновременно активна не более одна Sideband attempt на localId. Только владелец single-flight после одной network attempt транзакционно увеличивает `cleanup_attempt_count` ровно на 1 и записывает last/next timestamps; concurrent wake не удваивает backoff metadata. Due Sideband scan выбирает cleanup-marked non-terminal rows с известным `openai_session_id`, `cleanup_retry_exhausted_at IS NULL` и `cleanup_next_attempt_at <= now`. Каждая попытка делает transient Sideband attach → `session.close` с bounded wait; на `session.closed` вызывает `recordProviderClosed`. При transient failure retry использует backoff 1/2/4/8/16/30/60 секунд, затем cap 300 секунд + jitter. Marker/late-ID wake и startup используют тот же single-flight path; это не persistent Sideband connection.

Каждый worker tick/startup **сначала** выполняет expiry-only SQL scan по всем non-terminal rows с `cleanup_requested_at IS NOT NULL`, `cleanup_retry_expires_at <= now` и `cleanup_retry_exhausted_at IS NULL` — независимо от того, известен ли `openai_session_id`. Для такой row atomically фиксируется `cleanup_retry_exhausted_at`; если provider outcome не terminal, `state` становится/остаётся `unknown`. Expiry не ставит `close_confirmed`, не заполняет final, не фабрикует provider end и сам по себе не делает `lease_released_at` доказательством close. Только после expiry scan worker запускает due Sideband scan для rows с известным provider ID и неистёкшим cleanup retry. Таким образом marker-without-ID, у которого late result так и не появился, всё равно получает durable exhaustion/anomaly по 7-day deadline.
...
Cleanup и Usage outbox могут быть физически разными stores, но **capacity budget логически считается по уникальному `localId`**, а не по числу физических records. До external provider dispatch PR 2 резервирует budget envelope для `localId`; поэтому последующий cleanup этого уже-dispatched attempt никогда не требует дополнительного slot. Cleanup metadata priority-admitted и не может быть evicted/throttled ради usage; usage/checkpoint metadata того же `localId` coalesce-ится внутри того же envelope. При 1000 unique localIds новые provider attempts fail/pause before dispatch; существующая attempt всегда может записать cleanup. PR 3 обязан сохранять эту семантику при добавлении UsageOutbox. Cleanup metadata TTL default 7 days; недоступный IndexedDB даёт `cleanup_delivery_degraded`, exhausted/identity loss — явную anomaly, не silent success.
