...
| `SERVER_SHUTDOWN_DRAIN_MS` | 20000 | 20000; bounded drain provider-dispatched create handlers before worker stop |
...
`apps/api/src/server.ts` владеет lifecycle HTTP listener, SQLite adapter, `CleanupWorker` и in-flight create tracker. На `SIGTERM`/`SIGINT` выполняется один idempotent shutdown sequence. Create route и shutdown используют общий in-process **dispatch gate**: непосредственно перед external OpenAI call handler входит в короткую critical section, повторно проверяет `shuttingDown`, и только если gate открыт — durably коммитит `provider_request_dispatched_at`/reservation state и регистрирует себя как `provider-dispatched` in-flight handler. Shutdown закрывает тот же gate; после этого ни один registered-but-not-dispatched handler не может пересечь provider-dispatch boundary.

**Phase 1 — fence and stop accept.** Shutdown atomically выставляет `shuttingDown=true` под dispatch gate и инициирует `httpServer.close()`/эквивалент, чтобы перестать принимать новые HTTP connections/requests. Handler, который успел зарегистрировать local row/admission, но ещё не закоммитил dispatch marker, после закрытия gate creator не вызывает: transaction фиксирует safe no-dispatch/failed outcome и durable release reservation, затем HTTP возвращает shutdown/503. Этот путь не требует CleanupWorker.

**Phase 2 — drain provider-dispatched create handlers while cleanup remains alive.** Handlers, которые до закрытия gate уже durably записали `provider_request_dispatched_at`, продолжают bounded completion **при живых SQLite и CleanupWorker**. Worker перестаёт запускать unrelated periodic backlog drains/re-arm, но остаётся способен принимать direct cleanup wake/schedule от этих in-flight create handlers и завершать уже running cleanup commits. Поэтому late provider success, result-commit ambiguity или cleanup marker, возникшие во время drain, могут быть durably обработаны до worker stop. Drain ограничен `SERVER_SHUTDOWN_DRAIN_MS` (default 20000 ms). `infra/docker-compose.yml` в PR 2 задаёт `stop_grace_period: 30s`, то есть container grace обязана быть больше app drain budget.

Если provider-dispatched create handler завершился в drain window, он обязан либо вернуть/закоммитить normal result, либо durably записать существующий failure/cleanup/unknown contract до удаления из in-flight tracker. Если drain budget истёк на внешнем provider request без результата, shutdown не фабрикует failure/close и не повторяет create: already-durable dispatch marker остаётся источником `unknown` startup recovery. Если provider ID/result уже получен, но handler ещё коммитит его/cleanup marker, DB остаётся открыта до завершения bounded commit; нельзя останавливать worker раньше этого handoff.

**Phase 3 — stop cleanup worker, then DB.** Только когда provider-dispatched create tracker пуст или drain budget исчерпан с durable dispatched state, `CleanupWorker.stop()` прекращает remaining timers/queues, запрещает post-stop retry scheduling и bounded close/abort-ит active transient Sideband transports. Если shutdown abort прервал Sideband wait до provider outcome, worker не пишет `closed_observed`, `terminal_not_live`, retryable failure, attempt-count increment или новый backoff только из-за shutdown; durable marker/next остаются due для startup recovery. После worker quiesce закрывается HTTP server completion и **только затем** SQLite connection/resources.

Повторный signal может сократить оставшийся drain, но не обходить DB-consistent handoff: not-yet-dispatched handler всё равно не пересекает gate, а provider-dispatched row остаётся durably marked. Startup до допуска новых creates восстанавливает due cleanup/unknown rows как обычно. Container SIGTERM smoke обязан покрыть минимум два race: (1) signal между local registration и provider dispatch → provider calls=0, no-dispatch durable outcome; (2) signal во время provider create, затем success + result-commit failure → CleanupWorker/DB ещё живы, cleanup/unknown marker durable, после restart нет automatic create retry.
