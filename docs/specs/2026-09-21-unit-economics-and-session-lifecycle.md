...
| `cleanup_last_result`, `cleanup_last_error_code`, `cleanup_blocked_at` | Nullable cleanup-worker metadata. Result enum: `closed_observed`, `terminal_not_live`, `retryable_error`, `blocked_auth_config`. Error code — bounded normalized adapter/provider category, без raw message; blocked_at только для auth/config stop |
...
После expiry serialization due Sideband scan обрабатывает cleanup-marked non-terminal rows с известным provider ID, неистёкшим retry и `cleanup_next_attempt_at IS NULL OR cleanup_next_attempt_at <= now`; кандидаты проходят localId single-flight и global semaphore. Одновременно network-active cleanup attempts во всём API не превышают `CLEANUP_WORKER_CONCURRENCY`.

CleanupWorker нормализует Sideband outcome до четырёх классов. `closed_observed`: получен provider `session.closed`; вызвать `recordProviderClosed`, сохранить доступные final/reason/provenance. `terminal_not_live`: provider-specific authoritative result доказывает, что session больше не live/attachable; retry прекращается, `state=closed`, но `close_confirmed` остаётся false, `provider_final_seconds`/`provider_close_reason` не выдумываются, usage остаётся partial/unknown. `retryable_error`: timeout/network/5xx/rate-limit/unrecognized attach/protocol outcome; обычный backoff. `blocked_auth_config`: auth/permission/config/protocol incompatibility; tight retry прекращается в этом process, сохраняются `cleanup_last_result/error_code/blocked_at`, а startup/config reload может re-arm до cleanup TTL. Неизвестный provider error **по умолчанию retryable**, не terminal.
