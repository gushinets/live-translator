# Unit economics и жизненный цикл Live-сессий — спецификация v1.0
...
Слова **должен / нельзя** ниже задают предлагаемый контракт v1.0. Они станут принятыми требованиями после утверждения документа. Весь документ относится к этому MVP; он не объявляет клиентские отчёты достаточными для списаний денег.

## 1. Цель и границы

Для каждого логического разговора система должна показывать связанные попытки создания OpenAI-сессий, известный provider usage, полноту финализации, активное время интерпретатора и технические результаты перевода. Система не должна намеренно держать провайдерскую сессию открытой в фоне ради пятиминутного сохранения разговора.
...
**Вне области:** signup, платежи, подписки, баланс, серверно гарантированные paid quotas, full billing, аудиоархив, архив transcript, Redis, PostgreSQL, event bus, несколько экземпляров API и обязательный Sideband. Heartbeat в v1 не вводится. Автоматический rollover в бесконечную цепочку сессий также не вводится.
...
Persistent status: `active | paused | ended`. `active` означает разрешённый открытый conversation, а не утверждение, что аудио сейчас работает. `abandoned` — причина завершения, не четвёртый независимый lifecycle.
...
Каждая lifecycle-операция проверяет cookie → conversation → session. Для lifecycle используется оптимистическая версия `conversation.version`, увеличиваемая при pause/resume/end; запрос со старой версией получает 409 и не меняет состояние.

Обычный новый запуск во второй вкладке создаёт другой conversation. Нельзя автоматически открывать «последний активный разговор пользователя» из общего local storage. Resume возможен только для `paused` и валидной версии; два одновременных resume дают один успех и один конфликт. Поздний End старой вкладки не завершает возобновлённый conversation.
...
| `lease_expires_at`, `lease_released_at` | Admission reservation; не metering timestamps |
...
| `close_confirmed`, `provider_close_reason`, `app_end_reason` | Boolean и nullable причины, не один смешанный reason |
...
Индексы: PK обеих таблиц; UNIQUE `(anonymous_user_id, create_request_id)`; UNIQUE `openai_session_id` для non-null; UNIQUE `(conversation_id, generation)`; индекс sessions `(conversation_id, creation_requested_at)`; conversations `(anonymous_user_id, created_at)`; индексы `(status, resume_expires_at)` и sessions `(state, lease_expires_at)`. Ограничение одной progressing записи реализуется partial unique index или эквивалентной транзакционной проверкой под единственным writer.
...
| Создать провайдера | `POST /api/live/session` | Старый `sdp` + `conversationId`, `conversationVersion`, `liveSessionId`, `initialMode`, `startReason`; ответ сохраняет `session` и `transport`, добавляет `accounting` context |
...
| Resume | `POST /api/conversations/:id/resume` | `expectedVersion`; только retained `paused`; ответ новая version и policy. OpenAI создаётся отдельным следующим POST |
...
Для повторов lifecycle: запрос с `expectedVersion=v` считается повтором только когда текущая версия `v+1` и текущая операция/целевое состояние совпадают; иной конфликт — 409. Повтор pause не меняет `paused_at` и `resume_expires_at`. На end с уже ended возвращается текущее terminal состояние, не переписывается причина другим запросом.
...
Policy возвращается при create/read/resume conversation: `conversationRetentionMs`, `maxProviderSessionMs`, `maxConversationElapsedMs`, `sessionCloseTimeoutMs`, `backgroundSessionCloseEnabled`, `policyVersion`. Это серверные operational settings, а не ожидание, что Vite прочитает новые Docker env после сборки.
...
App totals передаются как накопленные значения для конкретного local session ID с `activity_report_seq`. Старый seq не откатывает app counters; финальный provider observation принимается независимо от этого seq. Нельзя суммировать conversation-cumulative metrics, повторённые в каждой provider session.
...
Первый conversation-report обязан показывать session attempts, known final subtotal, partial subtotal, unknown/conflict counts, active milliseconds, setup/visible pause milliseconds, число resume и причины завершения. Ratios при нулевом denominator — NULL, не 0/∞.
...
При valid server paused и deadline сначала атомарно claim resume (version). Далее — local media readiness, создание **одной** новой provider session, authoritative context, interpreter instructions, first steering и их ACK по существующим builders. Только затем gates-on и active interval. `authoritativeContextSent` новой сессии начинается false. Setup resume возвращает к незавершённому шагу, сохраняя принятые языки; interpreter resume не заставляет записывать A/B заново.

Если browser требует user gesture для mic/playback, UI остаётся paused с действием «Продолжить». Нельзя сообщать ready при blocked playback. Любой повторный hidden отменяет восстановление; rapid visible events делят одну операцию.

При reload восстановление того же conversation требует сохранённого snapshot и backend `paused`. Если pause report потерян и server ещё `active`, v1 не захватывает его автоматически и не создаёт потенциально вторую платную сессию: UI показывает невозможность безопасного автоматического resume и предлагает явное завершение/новый разговор. Если cookie утрачена, старые IDs не используются. Это намеренное ограничение MVP без heartbeat/серверной проверки liveness.
...
Admission остаётся memory registry, но durable reservation metadata хранится в ledger. При старте процесса незавершённые reservations восстанавливаются до разрешения новых созданий; создававшиеся в умершем процессе попытки становятся unknown, не повторяются автоматически. Expired reservation может освободить локальный слот; row сохраняет неизвестный provider outcome. Из ledger не выводится гарантированное число реально живых OpenAI-сессий.
...
**Gate G1 / после PR 3:** можно открыть один завершённый conversation и объяснить все созданные records и качество consumption; duplicate/out-of-order/closed-without-usage tests проходят; новый lifecycle ещё не включён.

**Gate G2 / после PR 5:** все deterministic lifecycle regressions проходят; normal hidden/resume не создаёт дубли, не теряет pair/context, не открывает old audio и не ломает same-speaker-consecutive routing. Фича доступна под отдельным flag.

**Gate G3 / после PR 6:** восстановление DB проверено; незавершённые rows после restart наблюдаемы; устройство/браузерные результаты и coverage сохранены; известные расхождения расхода объяснены или явно ограничивают применимость денежных выводов. Raw usage MVP может работать при незакрытом E1/E2, но не объявляется invoice-grade pricing model.
