...
`response.finish` может завершить только HTTP handler promise; он **не** завершает create-lifecycle tracking. После provider result commit in-memory entry переходит из network handler в `handoff_pending` и живёт до handoff ACK либо cleanup/timeout fence; authoritative copy — SQLite и startup его реконструирует. `creation_completed_at` само по себе не является tracker-removal/safe handoff condition. SIGTERM Phase 1 cleanup-fence-ит также result-committed, но handoff-unacknowledged attempts, даже если create HTTP handler уже вернул 201.
...
При старте процесса recovery/classification выполняется **до допуска новых созданий** и сначала применяет due deadline transitions через `expireResumeClaim()`/handoff watchdog. (1) `provider_request_dispatched_at IS NOT NULL` + `creation_completed_at IS NULL`, пока claim ещё valid: внешний результат не закоммичен, row остаётся honest ambiguous/`unknown` для accounting и external create не повторяется; если claim deadline уже достигнут, atomic expiry добавляет cleanup fence/closing до product rollback. (2) `creation_completed_at IS NOT NULL`, known provider ID, unacked, no cleanup, handoff+claim deadlines future: row сохраняется provisional `creating` вместе с reservation, browser может ACK. (3) expired handoff до claim expiry получает `handoff_timeout`; expired claim/retention/product deadline использует `expireResumeClaim()` и для любого dispatched non-terminal row гарантирует cleanup fence. (4) acknowledged provider `active` + valid pending resume claim сохраняется `resuming` и может `/resume/complete`; если claim истёк, тот же active provider atomically переводится в `closing` с cleanup fence до conversation rollback. Closed/released rows не rehydrate-ятся; no-dispatch invalid claim rollback-ится без provider cleanup.
