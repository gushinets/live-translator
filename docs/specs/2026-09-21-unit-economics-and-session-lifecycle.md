...
После cleanup marker SQL commit ответственность за provider termination переходит backend. PR 2 запускает server cleanup worker в том же API process: он выбирает due rows с `cleanup_requested_at IS NOT NULL`, известным `openai_session_id`, отсутствующим provider terminal proof и `cleanup_next_attempt_at <= now`; выполняет transient Sideband attach → `session.close` с bounded wait; на `session.closed` вызывает `recordProviderClosed`. При transient attach/close/timeout failure worker durable-обновляет attempt count/last/next timestamps и retry-ит с backoff 1/2/4/8/16/30/60 секунд, затем с cap 300 секунд + jitter. Marker commit и late provider-ID commit будят/schedule worker; startup восстанавливает due rows. Это не постоянное Sideband connection: каждая попытка transient.

Server cleanup retry продолжается независимо от product retention/max-conversation deadlines до одного из terminal proofs: definitive no-provider `failed`, provider `closed`/`close_confirmed=true`, подтверждённая provider-specific terminal result, либо `cleanup_retry_expires_at` (default 7 days). На retry expiry worker **не** объявляет close: фиксирует `cleanup_retry_exhausted_at`, оставляет provider outcome `unknown` и anomaly `cleanup_retry_exhausted`; admission release следует отдельному lease/cooperative contract. PR 6 может report/reconcile этот anomaly, но не владеет основным retry loop.
