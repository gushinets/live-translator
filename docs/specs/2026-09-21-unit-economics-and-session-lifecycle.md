...
| `cleanup_requested_at`, `cleanup_reason` | Nullable server UTC и nullable TEXT enum: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`, `server_shutdown`, `client_disconnected`, `handoff_timeout`, `resume_claim_expired`. После dispatch marker является activation fence; reason NULL iff marker NULL. `server_shutdown`, `client_disconnected`, `handoff_timeout`, `resume_claim_expired` — backend-only internal reasons и не принимаются из client cleanup payload |
...
| Подтвердить browser handoff | `POST /api/live/session/:localId/handoff` | Owner-authenticated idempotent ACK без provider ID. Для обычного initial/replacement start требует durable provider result, `cleanup_requested_at IS NULL` и `now < handoff_ack_deadline_at`. Для `start_reason=resume` **в той же SQL transaction** дополнительно требует matching `conversation.status=resuming`, `conversation.resume_attempt_id=localId`, `resume_outcome=pending` и все claim/retention/product deadlines строго future. Если любой resume deadline уже reached, transaction выполняет/переиспользует `expireResumeClaim()` semantics (cleanup fence для dispatched provider + claim rollback) и возвращает 409; handoff не коммитится. Если ACK выигрывает до deadline, atomically ставит `handoff_acknowledged_at` и provider `creating→active`; duplicate ACK idempotent. Cleanup/timeout/expiry-first marker ACK не оживляет и не очищает |
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent. Client allowlist: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`; backend-only `server_shutdown`/`client_disconnected`/`handoff_timeout`/`resume_claim_expired` rejected из payload. Idempotency key — `localId`, first committed reason wins. Pre-dispatch cleanup → definitive no-dispatch/release; post-dispatch marker→`closing` + retry expiry; ACK только после SQL commit |
...
| Resume complete | `POST /api/conversations/:id/resume/complete` | `expectedVersion` claim, `resumeAttemptId`, `providerStartedObservedAt`, `readyStage`; **в той же transaction** require matching pending claim, unexpired claim/product/retention deadlines, `cleanup_requested_at IS NULL`, `handoff_acknowledged_at IS NOT NULL` и provider `state=active`. Complete-before-handoff, provisional `creating`, cleanup/timeout-fenced или terminal provider → 409/no mutation; success `resuming→active` |
...
PR-2 provisional handoff watchdog durable и переживает API restart. Periodic/startup scan выбирает rows с `creation_completed_at IS NOT NULL`, known provider ID, `handoff_acknowledged_at IS NULL`, `cleanup_requested_at IS NULL`, `handoff_ack_deadline_at <= now`. CAS transaction ставит backend-only `handoff_timeout` cleanup marker, `state=closing`, immutable cleanup retry expiry и `cleanup_next_attempt_at=now`; reservation не release-ится без terminal proof. Handoff ACK transaction симметрично требует `cleanup_requested_at IS NULL` и `now < handoff_ack_deadline_at`. Для resume тот же transaction сериализован с `expireResumeClaim()`: до ACK проверяются matching `resuming`/pending claim и все claim/retention/product deadlines; если `now >=` любому effective deadline, ACK не может сделать provider active — expiry transition fence-ит/rollback-ит claim и возвращает conflict. На boundary выигрывает один SQLite writer/CAS: handoff-first до deadline → provider active; expiry/timeout/cleanup-first → closing/expired, late ACK возвращает fenced/current state и **никогда не очищает marker**.

`response.finish` может завершить только HTTP handler promise; он **не** завершает create-lifecycle tracking. После provider result commit network handler переходит в durable `handoff_pending`; authoritative copy — SQLite, startup его реконструирует. `creation_completed_at` само по себе не является safe handoff. **Graceful SIGTERM не ставит `server_shutdown` на result-committed, unacked row с `cleanup_requested_at IS NULL` и future handoff deadline**: shutdown только подтверждает, что durable provisional state committed, прекращает его in-memory timer и оставляет provider/session reservation для restart. Startup до admission re-arm-ит watchdog; если deadline истёк во время downtime, сразу применяет `handoff_timeout`. Result-committed row с уже expired handoff deadline fence-ится `handoff_timeout`, а не `server_shutdown`. Crash/ungraceful loss использует тот же durable startup classifier.
...
Claim expiry, request-time expiry check и maintenance используют **один transactional primitive `expireResumeClaim(...)`** над conversation + matching resume provider row. Если provider **не dispatched**, row может стать definitive no-dispatch `failed`, reservation отсутствует/освобождается по обычному no-provider rule, затем claim `expired` и conversation возвращается в `paused` либо `ended` по исходным retention/product deadlines. Если provider уже **dispatched и non-terminal** — независимо от того, provisional `creating`, handoff-acknowledged `active` или ambiguous без result — та же SQL transaction обязана сначала/одновременно обеспечить cleanup activation fence: first marker wins; если marker ещё NULL, записать `cleanup_requested_at=now`, backend-only `cleanup_reason=resume_claim_expired`, `state=closing`, immutable `cleanup_retry_expires_at=now+7d`, а при known `openai_session_id` ещё `cleanup_next_attempt_at=now`. После этого в **той же transaction** поставить `resume_outcome=expired`, очистить active claim pointer, увеличить conversation version и `resuming→paused/ended` без продления исходных deadlines.

`lease_released_at` не является заменой provider cleanup при claim expiry. Dispatched row не получает release только потому, что product claim истёк; release происходит только по существующим provider-terminal/cooperative release rules. Если cleanup marker уже был поставлен ранее (`hidden`, `server_shutdown`, etc.), expiry сохраняет first reason/immutable cleanup TTL и только завершает claim/product transition. SQL transaction исключает crash-gap «conversation уже paused/ended, а provider ещё без cleanup marker»; worker wake после commit — accelerator, а durable marker/next — recovery authority.

Далее клиент проверяет local media readiness, создаёт одну provider session с `liveSessionId=resumeAttemptId`, получает provisional 201, выполняет `setRemoteDescription` и подтверждает PR-2 handoff ACK. Только после committed handoff provider row становится `active`; conversation при этом остаётся `resuming`. Затем клиент восстанавливает authoritative context, interpreter instructions и first steering с ACK по existing builders. Setup resume восстанавливает только подтверждённый шаг настройки, не выдумывает готовый interpreter. `authoritativeContextSent=false` на новом provider; expectedSpeaker не вводится. Gates остаются закрыты. После требуемых для стадии ACK, `session.started` и готовности media клиент отправляет `/resume/complete` с claim version/ID. Backend в **одной complete transaction** принимает только pending matching claim до всех deadlines, с известными dispatch/result/OpenAI ID, `handoff_acknowledged_at IS NOT NULL`, provider `state=active` и `cleanup_requested_at IS NULL`. `readyStage` соответствует initialMode; started/readiness — клиентские observations. Complete-before-handoff/provisional creating получает 409 без мутации; timeout/cleanup-first/terminal provider никогда не переводит conversation в active. Backend фиксирует `resume_outcome=committed`, `resuming → active`, новую version и снимает active claim pointer. Только после commit ACK и повторной локальной проверки generation/visibility/deadlines/cleanup открываются gates.

Для resume handoff ACK и `expireResumeClaim()` используют одну и ту же SQLite serialization boundary. ACK не может опираться только на `handoff_ack_deadline_at`: repository transaction читает matching conversation/provider row и требует `status=resuming`, `resume_outcome=pending`, matching `resume_attempt_id`, `now < resume_claim_expires_at`, `now < resume_expires_at` и `now < product_deadline_at` когда они non-null. На точной границе `now >= deadline` expiry выигрывает: provider cleanup-fence-ится/claim rollback-ится и handoff получает 409. Если handoff CAS commits при `now < deadline`, provider может стать active, но последующий claim expiry до `/resume/complete` всё равно atomically cleanup-fence-ит его по `expireResumeClaim()`.
...
| Browser/API исчез / abort HTTP потерян / claim deadline reached | До deadline startup/read сохраняет valid provisional/handed-off pending claim по restart rules. **При claim/retention/product expiry** `expireResumeClaim()` atomically: no-dispatch row→failed; любой dispatched non-terminal provider→cleanup fence/`closing` (`resume_claim_expired` если first marker, `next=now` при known ID); затем `resume_outcome=expired`, conversation→paused/ended без продления. Crash не может разделить provider fence и claim rollback | Старый ID читает expired outcome; late handoff/complete не активируют; worker/restart продолжает provider cleanup |
...
Abort использует allowlisted категорию `media_not_ready`, `provider_creation_failed`, `webrtc_failed`, `restore_ack_failed`, `hidden`, `claim_timeout`, `interrupted_by_restart` или `user_end`, не произвольный текст ошибки. Abort/expiry увеличивает version, очищает active claim pointer, но не освобождает provider reservation как будто OpenAI уже остановлен. **Startup больше не прерывает все pending claims одинаково:** valid result-committed provisional/handed-off resume из таблицы выше сохраняется до собственных deadlines; только no-dispatch, ambiguous-no-result, expired/fenced cases rollback/abort-ятся. Никакого external POST startup не повторяет.
...
При старте процесса recovery/classification выполняется **до допуска новых созданий** и одинаково применяется после graceful SIGTERM restart и crash. (1) dispatched + `creation_completed_at IS NULL`: внешний результат не закоммичен, row остаётся honest ambiguous/`unknown` (либо closing при cleanup marker), external create не повторяется; graceful shutdown заранее ставит `server_shutdown` fence на такой still-in-flight create. (2) result-committed + known provider ID + unacked + no cleanup + future handoff (и для resume future claim/retention/product deadlines) сохраняется provisional `creating` вместе с reservation; graceful SIGTERM **не меняет его в closing**, startup re-arm-ит watchdog и browser может ещё ACK. (3) result-committed unacked с handoff deadline, истёкшим во время/до downtime, startup до admission ставит `handoff_timeout`; если resume claim/product/retention deadline достигнут раньше, применяется `expireResumeClaim()`/`resume_claim_expired`. (4) acknowledged provider `active` сохраняется; valid pending resume claim остаётся `resuming` и может `/resume/complete`, а reached claim deadline atomically cleanup-fence-ит provider и rollback-ит claim. Closed/released rows не rehydrate-ятся.
...
**Phase 1 — fence, stop accept, and classify create ownership.** Shutdown atomically выставляет `shuttingDown=true` под dispatch gate и инициирует `httpServer.close()`. Registered-but-not-dispatched handler после gate creator не вызывает: no-dispatch/release + 503. Для tracked **network in-flight** row с `provider_request_dispatched_at IS NOT NULL` и `creation_completed_at IS NULL` shutdown до drain ставит/сохраняет cleanup activation fence (`server_shutdown` if first reason, closing, immutable retry expiry; `next=now` если ID уже известен). Для durable **result-committed `handoff_pending`** row shutdown не ставит `server_shutdown`, если cleanup NULL и handoff deadline ещё future: row остаётся `creating`, reservation сохраняется, in-memory handoff timer прекращается, а startup re-arm-ит watchdog. Если handoff deadline уже reached на момент shutdown/startup, применяется `handoff_timeout`. Handoff-acknowledged `active` provider также не закрывается только из-за API restart; его product/claim lifecycle управляется отдельными deadlines.

**Phase 2 — drain cleanup-fenced network creates while CleanupWorker/DB remain alive.** Только dispatched rows без committed result, которые Phase 1 shutdown-fence-ил, продолжают bounded network completion. Late provider success persist-ит ID/result в fenced row, не возвращает SDP и ставит `cleanup_next_attempt_at=now`; worker принимает direct wake. Durable `handoff_pending`/acknowledged rows в этот drain не входят. Definitive provider create failure без ID допустим только при independently confirmed provider failure, не AbortSignal. Drain ≤ `min(SERVER_SHUTDOWN_DRAIN_MS, remaining global budget)`; timeout/global deadline abort-ит controller **после committed shutdown fence**, AbortError остаётся ambiguous provider outcome.
...
Container SIGTERM smoke обязан покрыть минимум четыре race: (1) signal между local registration и provider dispatch → provider calls=0/no-dispatch; (2) signal на dispatched/no-result create → `server_shutdown` fence + abort/late result cleanup; (3) **controlled SIGTERM after provider result/201 but before handoff ACK with future deadline** → row остаётся `creating`, cleanup marker NULL, restart re-arm-ит watchdog и ACK всё ещё возможен; если deadline проходит во время downtime, startup ставит `handoff_timeout`; (4) near-worst-case network drain + active Sideband stop → process exits before global/container deadlines.
