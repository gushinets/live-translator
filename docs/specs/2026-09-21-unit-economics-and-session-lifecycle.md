...
Cleanup и Usage outbox могут быть физически разными stores, но **capacity budget логически считается по уникальному `localId`**, а не по числу физических records. PR 2 вводит shared local metadata envelope keyed by `localId`; envelope резервируется durable **до начала provider-create fetch**, а local `create_dispatch_started_at` ставится непосредственно перед сетевой попыткой. Поэтому cleanup уже-started attempt никогда не требует дополнительного slot. Cleanup metadata priority-admitted и не может быть evicted/throttled ради usage; usage/checkpoint metadata того же `localId` coalesce-ится внутри того же envelope. При 1000 active envelopes новые provider attempts fail/pause before dispatch; существующая attempt всегда может записать cleanup.

`MetadataDeliveryBudget.reserve(localId)` — **hard precondition для любого нового billable provider dispatch**. Reserve должен durable-commit в IndexedDB до backend `/api/live/session`/provider-create request. Если IndexedDB недоступен, quota exceeded, transaction abort/timeout или envelope не удаётся подтвердить read-after-write, client **не** вызывает backend create, не acquires server admission и показывает recoverable local accounting/storage error (`metadata_storage_unavailable`/эквивалент). Повтор разрешён только после успешного reserve. Это fail-closed правило относится к новым attempts; уже существующая provider session, для которой envelope был успешно reserved ранее, при последующей деградации storage продолжает documented best-effort/direct cleanup HTTP fallback и не оставляется открытой только из-за local storage failure.

Reserve и dispatch связаны локальным state machine: `reserved → dispatch_started → delivery_pending → releasable`. `dispatch_started` нельзя ставить, пока reserve commit не подтверждён. Ошибка между reserve и network dispatch освобождает never-dispatched envelope по release predicate; ошибка после `dispatch_started` считается ambiguous до server read-back. IndexedDB-unavailable/quota-exceeded **до create** даёт zero backend/provider calls.
...
`apps/api/src/server.ts` владеет lifecycle HTTP listener, SQLite adapter и `CleanupWorker`. На `SIGTERM`/`SIGINT` выполняется один idempotent shutdown sequence. Сначала atomically выставляется `shuttingDown=true`: новые billable create requests fail 503 **до** provider dispatch, worker прекращает periodic/completion drains и не принимает новые wake/re-arm tasks. Уже начатые HTTP mutations могут закончить bounded commit; новые cleanup attempts не стартуют.

Затем `CleanupWorker.stop()` очищает timers/queues, запрещает post-shutdown `cleanup_next_attempt_at` scheduling и сигнализирует active transient Sideband transports на close/abort. Worker bounded ждёт уже running attempts не дольше `SESSION_CLOSE_TIMEOUT_MS` (или меньшего remaining shutdown budget). Если attempt успела получить provider terminal outcome и SQL commit **до** abort — commit сохраняется. Если shutdown прервал network wait до provider outcome, worker **не** записывает `closed_observed`, `terminal_not_live`, retryable failure, attempt-count increment или новый backoff только из-за process shutdown; ранее durable `cleanup_next_attempt_at`/marker остаются due (если next был claimed in-memory, durable row возвращается/остаётся immediately due) для startup recovery.

После worker quiesce/bounded abort закрывается HTTP server и только затем SQLite connection/resources. Повторный signal ускоряет force-exit только после попытки сохранить DB-consistent due state; он не фабрикует cleanup success. Startup до допуска новых creates восстанавливает due cleanup rows как обычно. Container SIGTERM smoke обязан показать: running Sideband attempt прерывается/закрывается bounded, DB не закрывается под active commit, row остаётся durably due, restart продолжает cleanup.
