...
| `cleanup_requested_at`, `cleanup_reason` | Nullable durable cleanup intent; reason allowlist как у cleanup route. Может быть записан после dispatch ещё до появления `openai_session_id`; не означает подтверждённый close |
...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent; `reason=response_not_received | primary_startup_failed | abandoned_connect`. Не создаёт provider, не требует актуальной product generation/version и не расходует creation limiter. Если request уже dispatched, но `openai_session_id` ещё NULL, атомарно сохраняет `cleanup_requested_at/reason` и отвечает как pending cleanup; если ID известен и primary close path недоступен, backend использует transient Sideband `session.close` |
...
`provider_closed` без seconds устанавливает `close_confirmed=true`, сохраняет `provider_final_seconds=NULL` и фиксирует `close_confirmation_source`. Для Sideband source событие наблюдает backend напрямую; для browser source это owner-authenticated forwarded provider event и оно **не** является независимой security/billing authority. Source merge монотонный: `NULL < browser < sideband`. Browser-first row может быть усилена поздним Sideband observation и заполнить ранее отсутствующий final/reason; поздний browser report никогда не понижает `sideband` обратно в `browser` и не переписывает Sideband reason. Для двух observations одинаковой strength первый non-null reason сохраняется, несовпадение отмечается bounded anomaly metadata. Numeric final merge остаётся независимым: missing final можно заполнить, exact duplicate идемпотентен, различающиеся finals идут по существующим conflict rules и не выбираются через `max()`.
