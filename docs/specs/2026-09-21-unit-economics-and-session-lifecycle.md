...
| `cleanup_requested_at`, `cleanup_reason` | Nullable server UTC и nullable TEXT enum: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`, `server_shutdown`. После dispatch marker является activation fence; reason NULL iff marker NULL. `server_shutdown` — backend-only internal reason и не принимается из client cleanup payload |
...
| Cleanup попытки | `POST /api/live/session/:localId/cleanup` | Owner-authenticated idempotent durable cleanup intent. Client reason allowlist: `response_not_received`, `primary_startup_failed`, `abandoned_connect`, `hidden`, `user_end`, `cancelled`, `replacement`; `server_shutdown` зарезервирован backend и rejected из client payload. Idempotency key — `localId`: first committed reason wins; поздние valid client requests ACK-ятся без переписывания. После dispatch marker→`closing`; ACK только после SQL commit |
